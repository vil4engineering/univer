# Campus

Student-facing OS — Engineering University **v2**.

Library = warehouse. `campus/` = navigate · track · Skills · learn.

## Start here

| Need | Go |
|------|-----|
| **Assistant manual** | [ASSISTANT_MANUAL.md](ASSISTANT_MANUAL.md) |
| v2 ratification | [V2_CHECKLIST.md](V2_CHECKLIST.md) |
| Charter v2 | [../01_CHARTER.md](../01_CHARTER.md) |
| Path Alpha (paused) | [paths/alpha.md](paths/alpha.md) |
| Senior roadmap (3–4 mo) | [ROADMAP_SENIOR.md](ROADMAP_SENIOR.md) |
| Org product (10–20 y) | [ORG_PRODUCT.md](ORG_PRODUCT.md) |
| Skill Factory | [skills/](skills/) |
| Career OS hybrid | [CAREER_INTEGRATION.md](CAREER_INTEGRATION.md) |
| Faculties | [faculties/](faculties/) |
| Language | [LANGUAGE.md](LANGUAGE.md) |
| Progress / Evidence | [PROGRESS.md](PROGRESS.md) |
| Topic template (Levels 1–3) | [TOPIC_TEMPLATE.md](TOPIC_TEMPLATE.md) |
| Layout / Display 0.1 | [LAYOUT_0.1.md](LAYOUT_0.1.md) · [DISPLAY_0.1.md](DISPLAY_0.1.md) |
| Labs · Library · Projects | [labs/](labs/) · [library/](library/) · [projects-map.md](projects-map.md) |

## Phase note

**v2 constitution in repo. Study paused** until you check human boxes on V2_CHECKLIST and Status unlocks Path lessons.
